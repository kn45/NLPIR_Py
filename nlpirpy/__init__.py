...
libFile = os.path.join(os.getcwd(), os.path.dirname(__file__)) + \
          '/libNLPIR64.so'
dll = CDLL(libFile)
...
class SegAtom(Structure):
    _fields_ = [("start", c_int32), ("length", c_int32),
                ...
                ("word_ID", c_int32), ("word_type", c_int32),
                ("weight", c_int32)]
...
ImportUserDict = loadFun('NLPIR_ImportUserDict', c_uint, [c_char_p, c_bool])
...
if not Init(os.path.join(os.getcwd(), os.path.dirname(__file__)),
            ENCODING.UTF8_CODE, ''):
...
def seg(paragraph):
    ...
    atoms = [i.strip().split('/') for i in result.split(' ')
             if len(i) >= 1 and i[0] != ' ']
